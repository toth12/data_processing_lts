...
def main(fragments):
	...
	for index,record in enumerate(fragments):
		...
		for i in range(int(record['start_sentence_index'])-100, int(record['end_sentence_index'])+100):
			...
			if (new_end_sentence_index is not None) and (new_end_sentence_index is not None):
				break

		#if the previous did not produce results
		sentences=folia_doc.sentences(i)
		...
		if (new_end_sentence_index is None) or (new_end_sentence_index is None):
